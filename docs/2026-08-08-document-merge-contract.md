...
경우는 없다 — 즉 원본의 이 run을 그대로 들고 오면 (a) 한 절에 secPr이
둘 생기는 구조적 모순이거나 (b) 삽입 위치가 index 0이면 대상의 페이지
설정을 조용히 원본 것으로 갈아치우는 부작용, 둘 중 하나다. **정책(v1
확정)**: 복사되는 문단에서 이 절-설정 run을 무조건 제거한다(대상의
secPr이 항상 그 절의 유일한 권위로 남는다) — 내장된 id 참조까지 재매핑할
근거·스코프가 없으므로 "재매핑" 대신 "제거"를 택했다. 병합 보고서의
`sectionPropertiesStripped`에 제거 횟수를 남긴다(무음 처리 금지).
...
- **`subjectIDRef`(connectLine의 스마트 연결선)** — DEV-013이 이미 저작
  ...
  손상 대신 명시적 실패).
- **`linkListIDRef`/`linkListNextIDRef`(연결된 글상자 체인)** — 실코퍼스
  근거 없음(스키마만 있음, 이번 조사에서도 실사용 0건 관측). 있으면
  거부.
...
**실측으로 찾은 결함 2건(설계서 작성 이후, 구현 중 발견)**:
