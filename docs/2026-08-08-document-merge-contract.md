...
|---|---|---|---|
| 글자 모양 유지 | charPr/fontfaces 재매핑(축1) | **항상 새 id로 별도 보존**("유지" 쪽 고정) | ~~아니오~~ **1라운드 실측으로 재평가 필요** — 아래 "판정 갱신" 절: 직접 서식은 이 축과 무관하게 항상 보존됨이 확인됐다, 이 축이 실제로 무엇을 가르는지(스타일 이름 충돌 해소로 추정)는 2라운드 대기 |
| 스타일 유지 | style 재매핑(축1) | **항상 새 id로 별도 보존**("유지" 쪽 고정) | 상동 — 판정 갱신 절 참조 |
...
| 문단 모양 유지 | paraPr 재매핑(축1) | **항상 새 id로 별도 보존**("유지" 쪽 고정) | 상동 — 판정 갱신 절 참조 |
...
## 판정 갱신 — 박스 COM InsertFile 4축 gold 1라운드 실측 (사이클 6.15)
...
설정)으로 `InsertFile`해 HWPX로 저장한 gold 6종을 확보했다. 아래
두 판정은 이 gold를 직접 열어 재확인한 것이다(단순 전달이 아니라
독립 재현 — `hh:charPr id="7"`가 실제로 `height="160000"`·
`textColor="#CC0000"`·`<hh:bold/>`를 갖는지, 6개 산출물 각각의
`Contents/section0.xml`/`header.xml`에 그 charPr 참조가 실제로
남아있는지 직접 파싱해 확인했다).
...
반증됐다. 4축이 무엇을 실제로 가르는지는 여전히 미해결이다(아래).
...
**미해결(2라운드 대기) — 서식 3축(글자/문단/스타일 모양 유지)의 실제
의미**: 6개 조합 중 `absorb`/`char`/`para`/`style`/`keepall` 5개의
`Contents/section0.xml`을 직접 diff했다 — **문단 id 난수 3곳을
제외하면 완전히 동일**(길이도 7455바이트로 같고, 그 외 바이트 단위
차이 0), `header.xml`도 5개 전부 바이트 동일. 즉 이 1라운드 표본에선
글자/문단/스타일 모양 유지 축을 켜고 꺼도 산출물에 관측 가능한 차이가
없었다. **가설**: docA/docB가 같은 스켈레톤이라 스타일 정의(이름·id
둘 다)가 애초에 동일했고, 이 축들은 "스타일 **이름이 충돌**할 때 어느
문서의 정의를 쓰는가"를 가르는 것이지 "서식이 살아남는가"를 가르는
게 아니다 — 판정 1이 이미 보여주듯 서식 생존은 축과 무관하기 때문에
이 가설이 유력하다. 팀장이 `docB2`(개요2 스타일을 궁서 24pt 초록으로
의도적으로 변조 — docA와 이름은 같고 정의는 다름)로 absorb/style/
char/keepall 4콤보를 추가 발사해 뒀다 — 박스가 다시 켜지면 회수,
갈리면 위 가설이 확정된다.

**재서술 — "v1이 한컴 기본과 3축 반대"였다는 이전 판정("의외의 발견"
문단)**: 이 진술은 우리 자신의 재매핑 정책(dedupe 없음 — 항상 새
id로 복제)과 한컴 UI 체크박스의 on/off를 곧이곧대로 대응시킨
것이었는데, 판정 1·미해결 항목이 보여주듯 **그 대응 자체가
근거박약이었다** — 한컴의 4축은(적어도 서식 생존이라는 축에서는)
직접 서식에 아무 영향이 없고, 우리 v1의 "항상 새 id로 별도 보존"도
정확히 그 직접 서식을 잃지 않는 결과를 낸다 — **직접 서식 기준으론
사실상 일치**한다. 진짜 차이가 있다면(2라운드가 확정할 경우) 그건
"스타일 **이름 충돌** 시 어느 정의가 이기는가"라는, v1이 애초에
회피한 축(우리는 절대 충돌 안 하게 항상 새 id를 쓴다)이지, "서식을
버리는가 살리는가"가 아니다 — 우선순위 판단(위 판정①~④ 근거)은
여전히 유효하다(gold 확보 실패는 소급 무효화됐지만, "확률로 구현하지
않는다"는 결론 자체는 안 바뀐다 — 이제는 진짜 gold가 있으니 2라운드가
그 축의 진짜 의미를 확정하면 그때 흡수 방향 구현을 재논의한다).

gold 위치(사설 경로, 벤더드 코퍼스 아님): `scratchpad/box-gold/
docA.hwpx`·`docB.hwpx`·`docB2.hwpx`(원본) +
`merged_{absorb,char,para,style,section,keepall}.hwpx`(6개 산출물).
2라운드(docB2 4콤보)·액션 프로브 결과는 팀장이 박스 재가동 후 추가
전달 예정 — 도착하면 이 절을 갱신한다.
