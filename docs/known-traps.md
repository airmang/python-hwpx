...
- **목록 기본 모양이 한/글 새 목록과 다르다.** `apply_list_format(kind="bullet")`의
  글머리표는 1수준 `-`, 2수준 `○`이고, 한/글이 새로 만드는 글머리표는 수준과 상관없이
  `●`다. 번호 목록은 1수준(`1.`)은 같고, 3수준은 python-hwpx가 `1.1.1.`, 한/글이 `1)`로
  그린다. 한/글처럼 보이게 하려면 글머리표는 `bullet_char="●"`를 준다(번호 수준의
  글자 모양은 아직 고를 수 없다). 기본값은 7.0에서 한/글에 맞춘다.
