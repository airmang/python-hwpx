...
라이브러리가 목차를 삽입할 때(`add_native_toc`), 항목의 쪽번호는 **naive한 추정치**로 채워집니다. 라이브러리는 한/글이 아니므로 실제 페이지네이션을 알 수 없습니다. 대신 제목을 자동으로 찾아 만든 목차는 필드에 `dirty="1"`을 세팅합니다(`headings`로 제목을 직접 준 경우는 아래 참조).
...
## 제목을 직접 준 목차는 기본이 dirty="0"

한/글은 dirty 목차를 다시 만들 때 제목 문단을 목차 필드의 수집 규칙(개요 스타일 등)대로 새로 모읍니다. `add_native_toc(headings=[...])`로 개요가 아닌 문단을 제목으로 준 목차를 dirty로 두면, 한/글이 문서를 열면서 그 항목과 링크를 모두 버립니다. 그래서 `headings`를 직접 주면 `dirty`의 기본값은 `False`입니다. 쪽번호까지 한/글이 다시 계산하게 하려면 제목 문단에 개요 스타일을 준 뒤 `dirty=True`를 넘기세요.
...
- `headings`로 개요가 아닌 문단을 제목으로 준 목차는 dirty로 두지 마세요. 한/글이 다시 만들면서 항목과 링크가 사라집니다(그래서 이때 기본값이 `dirty=False`).
