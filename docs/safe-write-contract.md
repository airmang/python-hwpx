...
> **이름 확정(4.0.0).** `save_to_path` · `save_to_stream` · `to_bytes`는 목적지별
> 명시 메서드로 **장기 확정**됐다 — 개명 재론은 없다. (구 `HwpxDocument.save()`
> 호환 래퍼는 4.0.0에서 제거됐다. [4.0.0 마이그레이션](migration-4.0.md) 참조.)
