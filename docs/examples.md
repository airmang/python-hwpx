...
## 5. 선언형 document plan과 양식 워크플로

선언형 document plan, 운영계획서 품질 프로필, 승인 양식 분석·적용·검증은
장르 판단과 실행 정책을 포함하므로 5.0부터 `python-hwpx-automation`이
소유합니다. 이 core 예제집에는 해당 응용 API의 실행 예제를 중복해서
싣지 않습니다.

설치, 최신 API, document-plan 및 form-fill의 end-to-end 예제는
[`python-hwpx-automation` 사용 사례](https://github.com/airmang/python-hwpx-automation/blob/main/docs/use-cases.md)를
참고하세요. 4.x `python-hwpx` 호출을 옮기는 경우에는
[5.0 마이그레이션 가이드](migration-5.0.md)에서 대응 경로를 먼저 확인하세요.
