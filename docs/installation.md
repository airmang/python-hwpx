...
CI가 CPython **3.10 · 3.11 · 3.12 · 3.13 · 3.14** 전 버전에서 전체 테스트
스위트를 실행합니다. 순수 파이썬 구현이며 유일한 필수 의존성 `lxml`도
3.13/3.14용 휠을 제공하므로 별도 빌드 도구 없이 설치됩니다. 새 CPython
마이너 버전은 전체 스위트가 통과하는 시점에 매트릭스와 분류자(classifier)에
추가합니다.
