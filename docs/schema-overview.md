...
## 주요 누락 요소 및 확장 필요 영역
* **런/컨트롤 세부 모델링:** ParaList 스키마에는 표, 컨트롤, 텍스트 장식, 변경 추적 등 수십 가지 요소가 정의돼 있습니다. 이 가운데 표와 누름틀은 이미 전용 표면을 갖췄고(`HwpxOxmlTable`, `HwpxDocument.list_form_fields()`·`fill_form_field()`), `hwpx.oxml`은 24개 서브모듈로 나뉘어 있습니다. 남은 것은 필드 코드와 나머지 인라인 컨트롤로, 아직 `GenericElement`에 머뭅니다.
* **헤더 매핑 테이블 확장:** 남아 있는 참조 목록(예: 사용자 정의 확장 리스트)은 여전히 `other_collections`에 저장되므로, 필요에 따라 추가 데이터클래스를 도입하고 상호참조 도우미를 늘려야 합니다.
* **추가 파트 지원:** 바탕쪽(`masterPage`), 문서 이력, 버전 파트는 이제 매니페스트 스캔을 거쳐 `HwpxDocument.master_pages`·`histories`·`version`으로 노출되며, XML 트리를 고친 뒤 `mark_dirty()`만 호출하면 저장에 반영됩니다. 남은 확장 대상은 그 밖의 선택 파트(첨부·미디어 메타데이터 등)입니다.
* **네임스페이스 및 버전 정비:** 코드가 2011/2016 네임스페이스를 기본값으로 사용해 2024 스키마와 어긋납니다. 패키지/도구 전반에서 최신 네임스페이스를 기본으로 맞추고, 구버전 문서 호환 전략을 정리해야 합니다.
* **스키마 검증·테스트 범위 강화:** 번들된 `header.xsd`·`section.xsd`는 루트 요소만 선언하고 나머지를 `<xs:any processContents="lax"/>`로 받는 **느슨한 구조 스키마**입니다. 즉 OWPML 전체를 검증하지 않으며, 스키마를 통과했다는 사실이 한컴이 문서를 연다는 뜻은 아닙니다. 파서는 선택적으로만 이 XSD를 적용하고 상위 API(`HwpxDocument`)는 호출하지 않습니다. 실제 안전 판정은 패키지 매니페스트 검사와 열림 안전 게이트(`validate_package`·`validate_editor_open_safety`)가 담당합니다. `validate_document()`는 이에 더해 번들된 전체 OWPML 스키마(`owpml-*.xsd`)로도 검사해, 한/글 문서에도 있는 편차를 뺀 위반을 경고로 알립니다(`docs/owpml-deviations.md`의 "전체 스키마 검사"). 헤더·섹션 외 파트의 전체 스키마 검사와 경고를 오류로 올리는 일이 남은 과제입니다.
