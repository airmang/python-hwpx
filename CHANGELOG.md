...
- `doc.text.plain()`·`markdown()`·`html()`(`hwpx.tools.exporter`)에 `list_labels` 인자(기본
  ...
  센다. `^N`은 1수준부터 그 수준까지의 번호를 숫자로 잇고(`1.2.`), 정의가 없는 번호는 모든
  수준을 그렇게 쓰며, 정의가 없는 글머리표는 `●`로 쓴다. 글자가 기호 글꼴 문자(사용자 정의
  영역의 U+F09F·U+F0A7 등) 하나뿐이면 한/글의 텍스트 저장처럼 `●`·`■` 등 유니코드 기호로 쓴다.
