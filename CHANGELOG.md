...
- 6.5.0 릴리스 노트의 "과거 셀 채움 페이지 흐름 사례 두 건의 글자 가림은 이번
  버전에도 남아 있습니다"를 정정한다. 편집한 셀의 줄배치 캐시만 지우는 지금 동작에서,
  셀을 채운 뒤 쪽 수가 바뀌는 공개 양식(`m2_corpus/form_002.hwpx`의 세 글자 채움 등)은
  글자 겹침 없이 한컴이 새로 배치한 것과 같게 그려진다. `form_002`가 10쪽에서 11쪽이
  되는 것은 결함이 아니다. 채운 셀이 체크박스와 한 줄을 나눠 써서 세 글자가 둘째
  줄로 넘어가기 때문이다. FormFit(`hwpx.form_fit`)은 이 셀을 "안 맞음"으로 판정한다.
