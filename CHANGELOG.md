...
- 표 칸 하나의 테두리를 바꾸면(`table.set_cell_borders`, `table.set_cell_border_fill`) 이웃 칸의
  맞닿은 변이 옛 선으로 남아, 한컴에서 두 선이 겹쳐 그려지던 것을 고친다. 칸마다 네 변을
  따로 가지고 맞닿은 변은 두 칸의 선이 다 그려지므로, 한컴의 칸 테두리 명령은 이웃 칸의 맞닿은
  변도 같은 선으로 바꾼다. 이제 python-hwpx도 이웃 칸의 맞닿은 변만 같은 선으로
  바꾼 테두리/배경 정의를 이웃에 준다. 이웃의 다른 변과 배경은 그대로다. 한컴처럼, 맞닿은 변이
  바꾼 칸의 변보다 긴 이웃(한 칸 옆의 병합 칸)은 바꾸지 않는다.
...
- 쪽 종류가 다른 머리말·꼬리말을 함께 쓰면 한컴에서 일부가 사라지던 문제를
  고친다. 머리말·꼬리말을 설정하면 한컴이 읽는 `hp:ctrl` 사본을 같은 쪽 종류만
