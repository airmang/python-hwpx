...
- 편집기 안전 검사(`validate_package`·`validate_editor_open_safety`)가 한컴이 열지
  ...
  - 구역 항목의 `opf:item@id`가 `section0`부터 번호가 이어지지 않음. 한컴은 구역을 이
    id의 번호 순서로 찾고(spine이나 파일 이름이 아니라) 하나라도 없으면 열지 않는다.
    번호는 이어지지만 spine 순서와 다르면 경고다(한컴은 id 순서로 읽는다). id는 spine이
    가리키는 것을 보고, 같은 구역 파트를 가리키는 다른 항목은 세지 않는다. `repair_repack`은
    구역 id를 spine 순서대로 다시 매긴다
