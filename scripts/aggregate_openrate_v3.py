...
* ``work/openrate-corpus-v3/manifest.json``       — what was generated, by what
* ``work/openrate-corpus-v3/verdicts_v3.jsonl``   — per-file Hancom open verdicts
* ``work/openrate-corpus-v3/render_verdicts_v3.jsonl`` — per-file PDF render verdicts
* ``work/openrate-corpus-v3/hancom_build.json``   — which Hancom judged the run,
...
* ``docs/openrate/report-v3.json``    — per-stratum figures with the rule-of-three
  bound; ``hancom_build`` and ``tool_versions`` are REQUIRED fields here —
  an aggregation without them refuses to publish.
* ``docs/openrate/verdicts_v3.jsonl`` — the raw receipts re-issued with a
...
V3_DIR = PYTHON_HWPX / "work" / "openrate-corpus-v3"
...
def main() -> int:
    ...
    verdicts = _read_jsonl(V3_DIR / "verdicts_v3.jsonl")
    renders = _read_jsonl(V3_DIR / "render_verdicts_v3.jsonl")
    ...
    fitz_path = V3_DIR / "fitz_check_v3.json"
    ...
    report = {
        "schemaVersion": "hwpx.openrate.report/v3",
    ...
    report_path = OUT_DIR / "report-v3.json"
    report_path.write_text(
        json.dumps(report, ensure_ascii=False, indent=1, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    receipts_path = OUT_DIR / "verdicts_v3.jsonl"
