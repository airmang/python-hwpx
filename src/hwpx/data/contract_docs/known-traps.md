...
- **새 문서는 빈 줄로 시작한다.** `HwpxDocument.new()`의 첫 문단은 구역 설정
  ...
  첫 줄부터 쓰려면 첫 글은 `doc.paragraphs[0].text = ...`로 넣는다. 7.0에서는 첫
  `add_paragraph()`가 이 빈 문단을 채우게 바뀐다(안정 API 문서의 「7.0 에서 바뀌는
  동작」).
...
- **글자처럼 취급한 표는 쪽에서 나뉘지 않는다.** 한컴은 글자처럼 취급한 표
  ...
  내려 그리며, 종이 아래 끝을 넘는 행은 그리지 않는다. 기본 행 높이(12.7 mm)와
  A4 기본 여백이면 19행부터 본문을 넘고, 21행부터는 종이 끝을 넘는 행이 잘린다.
  여러 쪽에 걸칠 표는 `table.set_treat_as_char(False)`로 흐르게 하라.
