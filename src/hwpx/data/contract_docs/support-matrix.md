...
| 페이지 레이아웃(용지·여백·머리말/꼬리말·쪽번호·단·줄번호·격자·요소 숨김) | Edit·Create·Render-verified(부분) | `doc.page`(6.8 트레인㉚ 등재, 코드 자체는 이전 사이클부터 존재 — 편집기 표면 인벤토리 트레인㉙이 찾을 때까지 캐파빌리티·매트릭스 어디에도 등재가 없었다). 19개 메서드 중 5개(`set_visibility`/`set_line_numbers`/`set_grid`가 만드는 `hp:visibility`/`hp:lineNumberShape`/`hp:grid`, `restart_page_number`/`hide_page_elements`가 만드는 `hp:newNum`/`hp:pageHiding`)만 실한컴 증거가 있다 — `docs/openrate/report-v4.json` authored-page-structure(15/15 render_checked)·`docs/openrate/report-v6.json` authored-pagecontrol(15/15 render_checked). **나머지(용지 크기·여백·머리말/꼬리말·쪽번호 서식·단)는 코드는 있고 동작하나 실한컴 증거가 없다** — 이 부분 표기는 정직하게 미실측으로 남긴다(전체를 Render-verified로 뭉뚱그리면 혼합 지원 영역 오염) |
