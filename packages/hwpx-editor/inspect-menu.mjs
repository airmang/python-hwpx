...
const data = await page.evaluate(() => {
  ...
  const menuRoots = candidates
    ...
    .map((btn) => {
      const nearest = btn.closest('div.absolute');
      return {
        label: btn.textContent.trim(),
        parentClass: nearest?.className ?? null,
        rect: (() => {
  ...
  const menuDivs = Array.from(document.querySelectorAll('div')).filter((d) => /\babsolute\b/.test(d.className) && /top-full/.test(d.className));
  return {
    menuRoots,
